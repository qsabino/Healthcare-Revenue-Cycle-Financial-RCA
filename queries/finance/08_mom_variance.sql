...
SELECT
    ...
    total_paid - LAG(total_paid) OVER (ORDER BY month) AS change_amount,
    (total_paid - LAG(total_paid) OVER (ORDER BY month)) /
        NULLIF(LAG(total_paid) OVER (ORDER BY month), 0) * 100 AS pct_change
